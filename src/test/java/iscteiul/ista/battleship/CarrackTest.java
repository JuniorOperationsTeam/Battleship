...
class CarrackTest {
    ...
    @Nested
    @DisplayName("Posições e comportamento de tiros")
    class PositionAndShooting {
        ...
        @Test
        @DisplayName("Disparos em todas as posições afundam a nau; tiros repetidos não alteram o estado")
        void shootingAndRepeatedShots() {
            ...
            // atira em cada posição e verifica hits
            for (IPosition p : positions) {
                assertFalse(p.isHit());
                carrack.shoot(new Position(p.getRow(), p.getColumn()));
                assertTrue(p.isHit());
                // tiro repetido não muda
                carrack.shoot(new Position(p.getRow(), p.getColumn()));
                assertTrue(p.isHit());
            }

            // após todos os hits, não deve flutuar
    ...
    class FactoryTests {
        ...
        @Test
        @DisplayName("Ship.buildShip devolve Nau para 'nau' e null para desconhecido")
        void buildShipFactoryProducesCarrack() {
            Ship s1 = Ship.buildShip("nau", Compass.EAST, new Position(0, 0));
            assertNotNull(s1);
            assertEquals("Nau", s1.getCategory());

            Ship s2 = Ship.buildShip("unknown", Compass.EAST, new Position(0, 0));
            assertNull(s2);
        }
    }
}
